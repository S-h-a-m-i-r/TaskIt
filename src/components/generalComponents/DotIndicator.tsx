...
const CustomCalendarDay = ({ date, tasks }: CustomDayProps) => {
  ...
  const statusDots = Object.keys(tasksByStatus).slice(0, 3).map(status => (
    <div
      key={status}
      className="h-1.5 w-1.5 rounded-full"
      style={{ 
        backgroundColor: statusConfig[status as keyof typeof statusConfig]?.color || '#8E8E93' 
      }}
    />
  ));
                    ...
                    >
                      <div className="w-full">
                        <div className="flex items-center">
                          ...
                          />
                          <AntTooltip title="Click to view task details" style={{background: 'green'}}>
                            <Text 
