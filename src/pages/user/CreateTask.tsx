...
import RecurringTaskSettings, {
  RecurringSettings,
} from "../../components/generalComponents/RecurringTaskSettings";
import useTaskStore from "../../stores/taskStore";
import { useFileUpload } from "../../hooks/useFileUpload";
import { DatePicker, message, Radio, RadioChangeEvent, Tooltip } from "antd";
...
const CreateTask = () => {
  ...
  const handleRecurringChange = (e: RadioChangeEvent) => {
    ...
    setFormData((prev) => ({
      ...
      recurringSettings: isRecurring ? prev.recurringSettings : undefined,
    }));
  };
  ...
  const handleRecurringSettingsSave = (settings: RecurringSettings) => {
    setFormData((prev) => ({
      ...
      recurrencePattern: settings.pattern as "Daily" | "Weekly" | "Monthly",
      recurrenceEndDate:
        settings.endType === "endBy" && settings.endDate
          ? settings.endDate.toDate()
          : null,
    }));
  };

  const handleRecurringSettingsRemove = () => {
    setFormData((prev) => ({
      ...prev,
      isRecurring: false,
      recurrencePattern: undefined,
  ...
  const handleSubmit = async () => {
    ...
    if (formData.isRecurring) {
      if (!formData.recurringSettings) {
        message.error("Please configure recurring settings");
        return;
      }
      ...
      if (
        ...
        !formData.recurringSettings.endAfterCount
      ) {
        message.error("Please specify the number of occurrences");
        return;
      }
    }
    // Due date validation removed for recurring tasks
    // Recurring tasks use their recurrence pattern for scheduling

    setLoading(true);
    try {
      // Create task first
      console.log("Form Data:", formData);
      console.log("Recurring Settings:", formData.recurringSettings);
      const result = await createTask({
        ...
        recurrenceEndDate:
          formData.recurrenceEndDate instanceof Date
            ? formData.recurrenceEndDate
            : null,
        recurringSettings: formData.recurringSettings,
        // Only send due date for non-recurring tasks
        ...(!formData.isRecurring &&
        formData.dueDate &&
        dayjs.isDayjs(formData.dueDate)
        ...
        if (selectedFiles.length > 0) {
          try {
            ...
            if (attached) {
              ...
              message.warning(
                "Task created but file attachment failed. Please try again."
              );
            }
          } catch (fileError) {
            // Task was created but file upload failed
            message.warning(
              "Task created but file upload failed. Please try uploading files again."
            );
  ...
  return (
    <div className="p-9 w-full flex flex-col gap-10">
            ...
            {formData.isRecurring && (
              <div className="bg-gray-50 p-5 rounded-lg border border-gray-200 space-y-4">
                    ...
                    {formData.recurringSettings ? (
                      <div className="text-sm text-gray-600">
                        <p>
                          ...
                          {formData.recurringSettings.pattern}
                          {formData.recurringSettings.pattern === "Monthly" &&
                            ` - ${formData.recurringSettings.monthlyDayOfWeek} ${formData.recurringSettings.monthlyDay}`}
                          {formData.recurringSettings.pattern === "Weekly" &&
                            formData.recurringSettings.weeklyDays &&
                            ` - ${formData.recurringSettings.weeklyDays.join(
                              ", "
                            )}`}
                        </p>
                        <p>
                      ...
                      <p className="text-sm text-gray-500">
                        No recurring settings configured
                      </p>
                    )}
                  </div>
                  <ButtonComponent
                    title="Configure Recurrence"
                    className="px-4 py-2 bg-primary-50 hover:bg-primary-200 text-white rounded-md text-sm font-medium"
                ...
                <div className="text-sm text-gray-600 bg-blue-50 p-3 rounded border border-blue-100">
                  ...
                  <p>
                    This task will automatically create new instances based on
                    your configured schedule. You'll be able to track all
                    recurring instances from your dashboard.
                  </p>
                </div>
              </div>
            )}
      ...
      <RecurringTaskSettings
        ...
        onRemove={handleRecurringSettingsRemove}
        initialSettings={formData.recurringSettings}
      />
    </div>
  );
};
