...
const AuthFooter = ({ tab }: AuthFooterProps) => {
  ...
  // Handle postMessage errors from Google OAuth iframe
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      // Only handle messages from Google OAuth domains
      if (
        event.origin.includes("accounts.google.com") ||
        event.origin.includes("ssl.gstatic.com")
      ) {
        try {
          // Handle any postMessage communication from Google OAuth
          if (event.data && typeof event.data === "object") {
            console.log("🔍 Google OAuth postMessage received:", event.data);
          }
        } catch (error) {
          console.warn("⚠️ Google OAuth postMessage error handled:", error);
        }
      }
    };
    ...
    const handleError = (error: ErrorEvent) => {
      if (error.message && error.message.includes("postMessage")) {
        console.warn(
          "⚠️ Google OAuth postMessage error caught:",
          error.message
        );
        // Don't show error to user as this is often a non-critical iframe communication issue
      }
    };
  ...
  const handleGoogleSuccess = async (
    ...
    try {
      ...
      console.log(
        "✅ Google Auth Debug - Credential received, length:",
      ...
      if (!res?.user) {
        console.error(
          "❌ Google Auth Error - Invalid response structure:",
          res
        );
        message.error(
      ...
      if (err && typeof err === "object") {
        ...
        if (error.response) {
          ...
          message.error(
            "Google authentication failed: Network error - please check your connection"
          );
  ...
  const handleGoogleError = (error?: { type?: string }) => {
    ...
    if (error) {
      ...
      if (error.type === "popup_closed_by_user") {
        ...
        message.error(
          "Google authentication popup was blocked. Please allow popups and try again."
        );
        ...
        message.error(
          "Google authentication invalid request. Please refresh and try again."
        );
      } else {
        message.error(
          `Google authentication failed: ${error.type || "Unknown error"}`
        );
      }
    } else {
