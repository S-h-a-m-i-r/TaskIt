...
export const request = async <T = unknown>({
  ...
  try {
    console.log(
      `🔍 Request Debug - Making ${method.toUpperCase()} request to:`,
      url
    );
    ...
    console.log(
      `✅ Request Debug - ${method.toUpperCase()} response received:`,
      response.data
    );
    return response.data;
  } catch (error) {
    console.error(
      `❌ Request Error - ${method.toUpperCase()} request failed for ${url}:`,
      error
    );
    ...
    if (errorResponse.response) {
      ...
      console.error(
        "❌ Request Error - Network error, no response received:",
        errorResponse.request
      );
    } else {
      console.error(
        "❌ Request Error - Request setup error:",
